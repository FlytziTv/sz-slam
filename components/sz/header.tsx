...
export default function Header() {
  ...
  return (
    <header className="border-b border-border flex flex-row items-center gap-2 px-2 fixed top-0 left-0 right-0 bg-background z-10">
      ...
      {page.map((item) => {
        ...
        return (
          ...
          >
            <p
              className={`group flex leading-none px-4 py-2 group-hover:bg-accent transition-colors duration-150 rounded-sm text-sm font-medium group-hover:text-foreground ${isActive ? "text-foreground" : "text-muted-foreground"}`}
            >
              {item.title}
            </p>
          </Link>
